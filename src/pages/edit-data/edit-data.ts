...
export class EditDataPage {
  ...
  getCurrentData(rowid) {
    this.sqlite.create({
      ...
      db.executeSql('SELECT * FROM gastos WHERE rowid=?', [rowid])
        .then(res => {
  ...
  updateData() {
    this.sqlite.create({
      ...
      db.executeSql('UPDATE gastos SET date=?,type=?,description=?,amount=? WHERE rowid=?',[this.data.date,this.data.type,this.data.description,this.data.amount,this.data.rowid])
        .then(res => {
          console.log(res);
          this.toast.show('Atualizado!', '5000', 'center').subscribe(
            toast => {
